(function () {
  ...
  var F_TODAY = 1 / 3;
  var FSTAR   = Math.pow(2, -7/3);  // ≈ 19.8% — peak of fully-tapered issuance
  var F_SAT   = 0.5;
  ...
  // Tapered issuance during the transition, c the transition coefficient.
  function annualInflationPctTaper(f, c) {
    if (f <= 0) return 0;
  ...
  function taperSeries(c) {
    ...
    for (var i = 0; i < N_POINTS; i++) {
      var f = F_MIN + i * step;
      ys.push(+annualInflationPctTaper(f, c).toFixed(4));
    }
    return ys;
  }
  ...
  var layout = {
    ...
    dragmode: false,
    title: { text: 'Annual issuance: current vs. linear taper phasing in', font: { size: 14 }, x: 0.5, xanchor: 'center' },
  ...
  var traces = [
    ...
    },
    {
      ...
      hovertemplate: 'Taper: %{y:.3f}%<extra></extra>',
    },
  ];
  ...
  var el = document.getElementById('lintaptrans-issuancecurve-chart');
