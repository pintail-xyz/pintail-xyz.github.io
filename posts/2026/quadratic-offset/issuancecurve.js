(function () {
  ...
  var F_TODAY = 1 / 3;
  var F_SAT   = 0.5;
  var F_PEAK  = F_SAT / 5;   // net issuance peaks at u = 1/5
  var N_POINTS = 500;
  var F_MIN    = 0.0;
  var F_MAX    = 0.50;
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    yNet.push(+(w * clApr(f) * f * 100).toFixed(4));
  }
  ...
  var layout = {
    ...
    dragmode: false,
    title: { text: 'Annual ETH issuance: current vs. quadratic-tapered net', font: { size: 14 }, x: 0.5, xanchor: 'center' },
  ...
  var traces = [
    { x: fs, y: yCurrent,
      name: 'Current gross (∝ √f)', type: 'scatter', mode: 'lines',
      line: { color: '#1565c0', width: 2.5 },
      hovertemplate: 'Current: %{y:.3f}%<extra></extra>' },
    { x: fs, y: yNet,
      name: 'Net (quadratic taper)', type: 'scatter', mode: 'lines',
      line: { color: '#e65100', width: 2.5 },
      hovertemplate: 'Net: %{y:.3f}%<extra></extra>' },
  ];
  ...
  var el = document.getElementById('quadratic-offset-issuancecurve-chart');
