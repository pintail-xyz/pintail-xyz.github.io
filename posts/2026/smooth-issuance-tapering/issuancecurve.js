(function () {
  ...
  var u0        = F_TODAY / F_SAT;
  var sqrtRatio = Math.sqrt(F_TODAY / F_SAT);

  var K_linear = 1 / (1 - u0                      * sqrtRatio);
  var K_smooth = 1 / (1 - (3*u0*u0 - 2*u0*u0*u0) * sqrtRatio);
  var K_quad   = 1 / (1 - (2*u0 - u0*u0)          * sqrtRatio);

  function annualInflationLinear(f) {
    if (f <= 0 || f >= F_SAT) return 0;
    var u = f / F_SAT;
    return K_linear * (clApr(f) - u * aprAtSat) * f * 100;
  }

  function annualInflationSmooth(f) {
    if (f <= 0 || f >= F_SAT) return 0;
    var u = f / F_SAT;
    return K_smooth * (clApr(f) - (3*u*u - 2*u*u*u) * aprAtSat) * f * 100;
  }

  function annualInflationQuad(f) {
    if (f <= 0 || f >= F_SAT) return 0;
    var u = f / F_SAT;
    return K_quad * (clApr(f) - (2*u - u*u) * aprAtSat) * f * 100;
  }
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    ySmooth.push(+annualInflationSmooth(f).toFixed(4));
    yQuad.push(+annualInflationQuad(f).toFixed(4));
  }
  ...
  var traces = [
    ...
    { x:fs, y:yLinear, name:'Linear (B ≈ 140)', type:'scatter', mode:'lines',
      line:{ color:'#2e7d32', width:2, dash:'dash' }, hovertemplate:'Linear: %{y:.3f}%<extra></extra>' },
    { x:fs, y:ySmooth, name:'Smoothstep (B ≈ 162)', type:'scatter', mode:'lines',
      line:{ color:'#00838f', width:2.5 }, hovertemplate:'Smoothstep: %{y:.3f}%<extra></extra>' },
    { x:fs, y:yQuad, name:'Quadratic (B ≈ 233)', type:'scatter', mode:'lines',
      line:{ color:'#b5420f', width:2.5 }, hovertemplate:'Quadratic: %{y:.3f}%<extra></extra>' },
  ];
  ...
  var el = document.getElementById('poly-issuancecurve-chart');
