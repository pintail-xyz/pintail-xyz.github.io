(function () {
  ...
  var sinToday  = Math.sin(Math.PI / 2 * F_TODAY / F_SAT);   // sin(π/3) = √3/2
  ...
  // Above f_sat the offset equals the full gross base reward → net yield = 0.
  // Below f_sat the sine/S-curve shape is applied via u = f / F_SAT ∈ [0,1].

  function clAprLinear(f) {
    if (f >= F_SAT) return 0;
    return K_linear * (clApr(f) - (f / F_SAT) * aprAtSat);
  }

  function clAprSine(f) {
    if (f >= F_SAT) return 0;
    return K_sine * (clApr(f) - Math.sin(Math.PI / 2 * f / F_SAT) * aprAtSat);
  }

  function clAprSCurve(f) {
    if (f >= F_SAT) return 0;
    var s = Math.sin(Math.PI / 2 * f / F_SAT);
    return K_scurve * (clApr(f) - s * s * aprAtSat);
  }
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    ySine.push(+(clAprSine(f) * 100).toFixed(4));
    ySCurve.push(+(clAprSCurve(f) * 100).toFixed(4));
  }
  ...
  var traces = [
    ...
    },
    {
      ...
      hovertemplate: 'Linear: %{y:.2f}%<extra></extra>',
    },
    {
      x: fs, y: ySCurve,
      ...
      hovertemplate: 'S-curve: %{y:.2f}%<extra></extra>',
    },
    {
      x: fs, y: ySine,
      ...
      hovertemplate: 'Sine: %{y:.2f}%<extra></extra>',
    },
  ];
  ...
  var el = document.getElementById('sine-yieldcurve-chart');
