(function () {
  ...
  // B' = 192 = 3*64: matches current yield at today's staking ratio
  var SCALE_192 = 192 / BASE_REWARD_FACTOR;   // = 3
  // B' = 256 = 4*64
  var SCALE_256 = 256 / BASE_REWARD_FACTOR;   // = 4

  var fs = [], yCurrent = [], yNet192 = [], yNet256 = [];
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    yNet192.push(+(SCALE_192 * w * base * 100).toFixed(4));
    yNet256.push(+(SCALE_256 * w * base * 100).toFixed(4));
  }
  ...
  var layout = {
    ...
    dragmode: false,
    title: { text: "Annual ETH issuance: BASE_REWARD_FACTOR calibration", font: { size: 14 }, x: 0.5, xanchor: "center" },
  ...
  var traces = [
    ...
    { x: fs, y: yNet192,
      name: "B' = 192 (match today)", type: "scatter", mode: "lines",
      line: { color: "#00838f", width: 2.5 },
      hovertemplate: "B'=192: %{y:.3f}%<extra></extra>" },
    { x: fs, y: yNet256,
      name: "B' = 256", type: "scatter", mode: "lines",
      line: { color: "#e65100", width: 2.5 },
      hovertemplate: "B'=256: %{y:.3f}%<extra></extra>" },
  ];
  ...
  var el = document.getElementById("linear-taper-calibration-chart");
