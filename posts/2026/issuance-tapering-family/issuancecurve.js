(function () {
  ...
  function annualIssuanceMETHBeta(f, beta) {
    if (f >= 0.5) return 0;
    return Math.pow((0.5 - f) / (0.5 - F0), beta) * annualIssuanceMETH(f);
  }
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    yHalf.push(+annualIssuanceMETHBeta(f, 0.5).toFixed(4));
    yOne.push(+annualIssuanceMETHBeta(f, 1.0).toFixed(4));
    yTwo.push(+annualIssuanceMETHBeta(f, 2.0).toFixed(4));
  }
  ...
  var traces = [
    ...
    },
    {
      ...
      hovertemplate: 'β=½: %{y:.3f} M ETH<extra></extra>',
    },
    {
      x: fs, y: yOne, name: 'β = 1 (linear)',
      type: 'scatter', mode: 'lines',
      line: { color: '#00897b', width: 2.5 },
      hovertemplate: 'β=1: %{y:.3f} M ETH<extra></extra>',
    },
    {
      x: fs, y: yTwo, name: 'β = 2 (quadratic)',
      type: 'scatter', mode: 'lines',
      line: { color: '#6a1b9a', width: 2.5 },
      hovertemplate: 'β=2: %{y:.3f} M ETH<extra></extra>',
    },
  ];
