(function () {
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    yBlended.push(+(blendedApr(f) * 100).toFixed(4));
  }
  ...
  var layout = {
    ...
    shapes: [
      ...
      { type: 'line', x0: 0, x1: F_SAT * 100,
        y0: R_STAR * 100, y1: R_STAR * 100,
        line: { color: '#e57373', width: 1, dash: 'dash' } },
    ],
    annotations: [
      ...
      { x: 1, xanchor: 'left',
        y: R_STAR * 100, yanchor: 'bottom', yshift: 4,
        text: 'r* = rₛₐₜ ≈ ' + (R_STAR * 100).toFixed(2) + '%', showarrow: false,
        font: { size: 11, color: '#e57373' } },
    ],
    showlegend: true,
    legend: { x: 0.55, y: 0.98, bgcolor: 'rgba(255,255,255,0.8)' },
    dragmode: false,
    title: { text: 'CL yield: current vs. quadratic-blended curve', font: { size: 14 }, x: 0.5, xanchor: 'center' },
  ...
  var traces = [
    ...
    { x: fs, y: yBlended,
      ...
      hovertemplate: 'Blended: %{y:.2f}%<extra></extra>' },
  ];
  ...
  var el = document.getElementById('blended-yieldcurve-chart');
