(function () {
  ...
  var F_TODAY   = 1 / 3;
  var F_SAT     = 0.5;
  var FSTAR_SUB = 0.1283;  // peak of subtractive issuance: solve u^(3/2)*(10-9u)=1 => u*≈0.2565
  ...
  // Issuance = r(f) * f, as % of total ETH supply per year
  function annualInflationPct(f) {
    if (f <= 0) return 0;
    return BASE_REWARD_FACTOR * EPOCHS_PER_YEAR * Math.sqrt(f / (TOTAL_ETH_SUPPLY * 1e9)) * 100;
  }

  // Subtractive naive issuance: (r(f) - r_sat*f*(5-6f)) * f
  function annualInflationPctSubNaive(f) {
    if (f <= 0 || f >= F_SAT) return 0;
    var netApr = clApr(f) - aprAtSat * f * (5 - 6 * f);
    if (netApr <= 0) return 0;
    return netApr * f * 100;
  }

  // Subtractive compensated issuance at multiplier K
  function annualInflationPctSubComp(f, K) {
    return K * annualInflationPctSubNaive(f);
  }
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    yNaive.push(+annualInflationPctSubNaive(f).toFixed(4));
    yComp128.push(+annualInflationPctSubComp(f, K_2).toFixed(4));
    yComp256.push(+annualInflationPctSubComp(f, K_4).toFixed(4));
  }
  ...
  var layout = {
    ...
    dragmode: false,
    title: { text: 'Annual issuance: current vs. quadratic taper (B=128, B=256)', font: { size: 14 }, x: 0.5, xanchor: 'center' },
  ...
  var traces = [
    ...
    },
    {
      ...
      hovertemplate: 'Naive quadratic taper (B=64): %{y:.3f}%<extra></extra>',
    },
    {
      x: fs, y: yComp128,
      ...
      hovertemplate: 'Compensated quadratic taper (B=128): %{y:.3f}%<extra></extra>',
    },
    {
      x: fs, y: yComp256,
      ...
      hovertemplate: 'Compensated quadratic taper (B=256): %{y:.3f}%<extra></extra>',
    },
  ];
