(function () {
  ...
  function clAprNaive(f) {
    var base = clApr(f) - (f / F_SAT) * aprAtSat;
    return base > 0 ? base : 0;
  }

  function clAprNaive128(f) {
    var net = 2 * (clApr(f) - (f / F_SAT) * aprAtSat);
    return net > 0 ? net : 0;
  }
  ...
  for (var i = 0; i < N_POINTS; i++) {
    ...
    yNaive128.push(+(clAprNaive128(f) * 100).toFixed(4));
  }
  ...
  var layout = {
    ...
    dragmode: false,
    title: { text: 'CL yield: current vs. linear taper', font: { size: 14 }, x: 0.5, xanchor: 'center' },
  ...
  var traces = [
    ...
    },
    {
      ...
      hovertemplate: 'Naive linear taper (B=64): %{y:.2f}%<extra></extra>',
    },
    {
      x: fs, y: yNaive128,
      ...
      hovertemplate: 'Compensated linear taper (B=128): %{y:.2f}%<extra></extra>',
    },
  ];
  ...
  var el = document.getElementById('intuition-yieldcurve-chart');
